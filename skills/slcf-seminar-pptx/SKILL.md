...
`assets/plan-template.yaml`을 복사해서 채웁니다. 스키마 상세는 `references/plan-schema.md`. 두 가지 sample:
- `samples/example-plan.yaml` — 책 전체 발표 (full mode, 26 슬라이드, 13 챕터)
- `samples/example-plan-ch10-scoped.yaml` — 단일 챕터만 발표 (scope=chapter, 18 슬라이드, mixed focus)
